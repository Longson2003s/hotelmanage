...
where (pdp.ngayDen >= '2019-12-26' or pdp.ngayDi  <='2019-12-11')--di-den
...
where (pdp.ngayDen <= '2019-12-26' and pdp.ngayDi  >='2019-12-11') and p.maPhong = 8 --di-den
...
where (pdp.ngayDen  <= '2019-12-26' and pdp.ngayDi  >='2019-12-11')--đi - đến
-- list pdp cua 1 phong tu ngay den - ngay di -> dsPDP
select pdp.*, kh.hoTen,kh.CMND,
	case
		when pdp.ngayDi = '2019-10-23' then 4 -- sap check out
		when pdp.ngayDen = '2019-10-23' then 2 -- dang check in
		when pdp.ngayDen < '2019-10-23' then 3 -- dang su dung
		else 1 -- da dat
	end as tinhtrang
...
where (
	( pdp.ngayDi  >='2019-10-23') -- ngay hien tai
	and p.maPhong = 2
)
...
select distinct pdp.maPhong,
	case
		...
		when pdp.ngayDen = '2019-11-20' then 2 -- den han check in
		when pdp.ngayDi = '2019-11-20' then 4 -- den han check out
		else 3 -- dang su dung
	end as tinhtrang
from  PhieuDatPhong as pdp right join Phong as p on pdp.maPhong = p.maPhong
where ( (pdp.ngayDen <= '2019-11-20' and pdp.ngayDi  >='2019-11-20')) --di-den
...
--QUẢN LÝ Khách Hàng--BY ĐINH HOÀNG HẢI ĐĂNG
...
--QUẢN LÝ DỊCH VỤ --BY ĐINH HOÀNG HẢI ĐĂNG
...
having DATEPART(mm,pdp.ngayDen) = 11 and  DATEPART(yyyy,pdp.ngayDen) = 2019
-- THỐNG KÊ SỐ LƯỢNG PDP TỪNG PHÒNG
select p.maPhong, tt = Count(p.maPhong) from Phong as p
join PhieuDatPhong as pdp on pdp.maPhong = p.maPhong
where DATEPART(MM,pdp.ngayDen) = 12 and  DATEPART(yyyy,pdp.ngayDen) = 2019
group by p.maPhong
